...
- Score 4-5: GO for broader operational use, subject to normal compliance and oversight processes. Score 4–5 additionally requires (a) documented agency data platform availability assessment (preferring existing governed assets) and (b) a defined pre-launch baseline plus active post-launch KPI measurement program with scheduled reporting.
...
- Unstructured documents are ingested through semantic parsing (Proto-SPOs with provenance and structure — not chainsaw chunking), exposed via MCP ingestion servers.
...
- Data ingestion decisions explicitly prefer assets already present and governed in the agency data platform; program-supplied data is the exception, not the default.
...
- Has a formal check been performed to determine whether the required data is already available and governed in the agency data platform, or must new program-supplied pipelines be created?
